...
exports.findUsersWithAccount = async (req, res) => {
    const info = await user.find().populate('account').sort({
        "_id": -1
    })
    res.status(200).json({
        info: info
...
exports.findUsersWithTransaction = async (req, res) => {
    const info = await user.find().populate('transaction').sort({
        "_id": -1
    })
    res.status(200).json({
        info: info
...
exports.getUserDetails = async (req, res) => {
    ...
    const decode = await jwt.verify(token, config.Usercode)
...
exports.findOneUser = async (req, res) => {
    ...
    if (User) {
        ...
        res.json({
            message: 'No user with such account number'
        })
    }
}